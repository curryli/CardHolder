...
public class ScoreMapper extends Mapper<Object, Text, Text, Text>{
	...
	// 输入   卡MD5，发卡机构，发卡机构名称，卡等级， 近6月广义消费金额，近6月中有广义消费的月份数目， 近6月广义月均消费金额
	
	//这边其实就是只要map的结果，不要reduce了，最简单的方法就是在ScoreDriven里面设置job.setNumReduceTasks(0);这样就把map的结果直接输出到指定的输出目录中去了
	public void map(Object key, Text value, Context context) throws IOException, InterruptedException{
		...
		String cardId = tokens[0].trim();  //卡md5
		String cs_trx_amt_l6m = tokens[5].trim(); //近6月广义消费金额
		String month_count = tokens[6].trim(); //近6月中有广义消费的月份数目
		float avg_amt = Float.parseFloat(tokens[7].trim()); // 近6月广义月均消费金额
