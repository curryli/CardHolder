 ...
 * */
public class CardLevlDriven extends Configured{
...
//		conf.set("dfs.namenode.rpc-address.nameservice1.namenode684", "bB0103003:8020");
		...
		job.setJarByClass(CardLevlDriven.class);
		
		job.setMapperClass(CardLevMapper.class);
		job.setReducerClass(CardLevReducer.class);
...
//		System.exit(job.waitForCompletion(true) ? 0 : 1);
		boolean tag = job.waitForCompletion(true);
		if(tag){
			FileUtil.copyMerge(fs,new Path(otherArgs[1]+"_temp"),fs,new Path(otherArgs[1]),true,conf,null);
		}
		System.exit(0);
