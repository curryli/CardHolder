...
public class MapAPPMapper extends Mapper<Object, Text, Text, Text>{
	...
	public void map(Object key, Text value, Context context) throws IOException, InterruptedException{
		...
		if(tokens[36].replaceAll("\"", "").trim().equals(""))          //卡号为空
			return;
		else{
...
//				return;
			...
			//7.受理机构代码
			String acptNum = tokens[33].replaceAll("\"", "").trim();
...
//			} 
			...
			if(fwd_ins_id_cd.equals("00010344"))									//如果是境外交易，剔除
				return;
			...
			//24.转入卡号
			String tfr_in_acct_no  = tokens[109].replace("\"", "").trim();
			sb.append(tfr_in_acct_no+",");
			
			//25.转出卡号
			String tfr_out_acct_no  = tokens[110].replace("\"", "").trim();
			sb.append(tfr_out_acct_no+",");
			
			//26.清算金额                        检查清算金额 tokens[134] 全数字
...
//				System.out.println("rcv_settle_at is illegal，not all numerous!" + rcv_settle_at);
			...
			String city = Constant.getCity(districtNum);
			...
			context.write(new Text(province+","+year),new Text("1"));
		}
	...
	public boolean isTransferAcountTrans(String trans_id){
		if(trans_id.equals("S02") || 
				trans_id.equals("S25") ||
				trans_id.equals("33"))
			return true;
		else 
